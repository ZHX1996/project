...
parser.add_argument('--data_dir', type=str, default='E:/Project/DL/cifar10_data',
...
def _activation_summary(x):
    ...
    tf.summary.histogram(tensor_name + '/sparsity',
...
def _variable_with_weight_decay(name, shape, stddev, wd):
    # helper to create an initializer variable with weight decay.
    dtype = tf.float16 if FLAGS.use_fp16 else tf.float32
...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay('weight',
                                            ...
                                            stddev=5e-2,
                                            wd=0.0)
    conv = tf.nn.conv2d(images, kernel, [1, 1, 1, 1], padding='SMAE')
    ...
    norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norl1')
